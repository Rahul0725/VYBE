...
export const useThemeStyles = (activeTheme: TemplateConfig) => {
  const themeFlags = useMemo(() => {
    return {
      ...
      isComicBio: activeTheme.id === 'comic_bio',
    };
  }, [activeTheme.id]);
  ...
  const styles = useMemo(() => {
    ...
    const card = {
      ...
      borderRadius: isComicBio ? '18px' : isCreatorBlackPro ? '28px' : isSunsetGradientPro ? '20px' : isClassicSolidPro ? '12px' : (isCleanLightPro || isSoftDarkPro || isSolidLightCore || isAccentSolidMinimal) ? '14px' : (is3DElevationStack || isEmberContrastPro) ? '16px' : (isFloatingPanelsPro || isRoyalEmber3D) ? '18px' : isUltraDepthPurple ? '20px' : isElegantPortraitCard ? '50%' : (activeTheme.buttonStyle === 'pill' ? '9999px' : activeTheme.buttonStyle === 'square' ? '0px' : '16px'),
  ...
  const classes = useMemo(() => {
              ...
              : isClassicSolidPro
                ...
                : isCleanLightPro
                  ...
                  : isSoftDarkPro
                    ...
                    : isComicBio
                      ? 'w-[110px] h-[110px] border-[4px] border-black'
                  : isLiquidGlass  
      ...
      isLiquidGlass 
        ...
        : activeTheme.containerGlass || isGlassElite
          ...
          : isElegantPortraitCard
            ...
            : isComicBio
              ? 'max-w-[420px] bg-white border-[4px] border-black rounded-[25px] p-[35px] shadow-[10px_10px_0_#000] flex flex-col relative'
            : 'max-w-2xl'
    }`;

    const name = `text-3xl font-black tracking-tight mb-2 ${isNeonPulse ? 'neon-text-flicker uppercase' : ''} ${isEditorialLuxe ? 'font-serif text-4xl' : ''} ${isClassicSolidPro ? 'text-2xl font-bold tracking-normal' : ''} ${isCleanLightPro ? 'text-2xl font-bold text-black tracking-tight' : ''} ${isSolidLightCore ? 'text-2xl font-bold text-black' : ''} ${isAccentSolidMinimal ? 'text-2xl font-bold text-white tracking-tight' : ''} ${is3DElevationStack ? 'text-2xl font-bold text-white tracking-tight' : ''} ${isFloatingPanelsPro ? 'text-3xl font-bold text-white tracking-tight drop-shadow-lg' : ''} ${isRoyalEmber3D ? 'text-3xl font-bold text-white tracking-tight' : ''} ${isEmberContrastPro ? 'text-3xl font-bold text-white tracking-tight' : ''} ${isUltraDepthPurple ? 'text-3xl font-bold text-white tracking-tight' : ''} ${isElegantPortraitCard ? 'text-2xl font-serif text-[#8E4F4B] mt-4 font-medium tracking-wide' : ''} ${isYellowHeroCard ? 'text-3xl font-bold text-[#222222] mt-12' : ''} ${isBrushArtistCard ? 'text-[32px] font-["Caveat",cursive] text-[#1A1A1A] mt-4 leading-none' : ''} ${isSunsetMotionProfile ? 'text-[28px] font-bold text-white mt-4 tracking-tight' : ''} ${isNeonPulseProfile ? 'text-2xl font-bold text-[#1F1F1F] mt-4 tracking-tight' : ''} ${isComicBio ? 'text-[32px] mt-[10px]' : ''}`;

    const bio = `text-lg max-w-md font-medium ${isEditorialLuxe ? 'font-light italic opacity-80' : ''} ${isClassicSolidPro ? 'text-base opacity-80 font-normal' : ''} ${isCleanLightPro ? 'text-base text-gray-500 font-normal' : ''} ${isSoftDarkPro ? 'text-base opacity-70 font-normal' : ''} ${isSolidLightCore ? 'text-base text-[#555555] font-normal' : ''} ${isAccentSolidMinimal ? 'text-base text-white/80 font-normal' : ''} ${is3DElevationStack ? 'text-base text-gray-400 font-normal' : ''} ${isFloatingPanelsPro ? 'text-base text-slate-400 font-medium' : ''} ${isRoyalEmber3D ? 'text-base text-[#E6E6FA] font-medium opacity-70' : ''} ${isEmberContrastPro ? 'text-base text-white/90 font-medium' : ''} ${isUltraDepthPurple ? 'text-base text-white/80 font-medium' : ''} ${isElegantPortraitCard ? 'text-xs uppercase tracking-[0.2em] text-[#8E4F4B]/70 mt-1' : ''} ${isYellowHeroCard ? 'text-base font-medium text-[#666666] mt-1' : ''} ${isBrushArtistCard ? 'text-base font-medium text-[#1A1A1A] mt-2' : ''} ${isSunsetMotionProfile ? 'text-base font-medium text-white/80 mt-1' : ''} ${isNeonPulseProfile ? 'text-[15px] font-normal text-[#4A4A4A] leading-relaxed mt-1' : ''} ${isComicBio ? 'bg-white border-[3px] border-black rounded-[20px] py-[10px] px-[14px] my-[16px] mx-auto inline-block relative after:content-[\'\'] after:absolute after:-bottom-[12px] after:left-[30px] after:w-0 after:h-0 after:border-l-[10px] after:border-l-transparent after:border-r-[10px] after:border-r-transparent after:border-t-[12px] after:border-t-black' : ''}`;

    const linksContainer = `w-full ${activeTheme.layout === 'grid' ? 'grid grid-cols-1 sm:grid-cols-2 gap-4' : activeTheme.layout === 'profile-card' ? 'flex justify-center gap-6 mt-8' : isYellowHeroCard ? 'space-y-4 mt-8' : isBrushArtistCard ? 'space-y-[18px] mt-8' : isSunsetMotionProfile ? 'space-y-[18px] mt-10' : isNeonPulseProfile ? 'space-y-5 mt-8' : isComicBio ? 'flex flex-col gap-[12px]' : 'space-y-4'} ${isFloatingPanelsPro || isRoyalEmber3D || isUltraDepthPurple ? 'perspective-[1000px]' : ''}`;
    ...
    const getInitial = () => {
      ...
      if (isComicBio) return { opacity: 0, y: 20 };
    ...
    const getAnimate = () => {
      ...
      if (isComicBio) return { opacity: 1, y: 0 };
    ...
    const getTransition = (index: number) => {
      ...
      if (isComicBio) return { duration: 0.3, delay: 0.05 * index };
    ...
    const getWhileHover = () => {
      ...
      if (isComicBio) return { x: 3, y: 3, boxShadow: '2px 2px 0 #000' };
